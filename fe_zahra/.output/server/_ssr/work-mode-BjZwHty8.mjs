import { r as __toESM } from "../_runtime.mjs";
import { m as require_react } from "../_libs/@base-ui/react+[...].mjs";
import { o as getVendorById } from "./db-_r_BwSlB.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/work-mode-BjZwHty8.js
var import_react = /* @__PURE__ */ __toESM(require_react());
/** Fixed head-office coordinates used when an employee checks in as "Office Base". */
var OFFICE = {
	name: "Sentinel HQ Jakarta",
	lat: -6.2088,
	lng: 106.8456,
	radius: 150
};
/** Great-circle distance between two coordinates, in meters (rounded). */
function haversine(lat1, lng1, lat2, lng2) {
	if (!lat1 && !lng1) return 0;
	const R = 6371e3;
	const toRad = (d) => d * Math.PI / 180;
	const dLat = toRad(lat2 - lat1);
	const dLng = toRad(lng2 - lng1);
	const a = Math.sin(dLat / 2) ** 2 + Math.cos(toRad(lat1)) * Math.cos(toRad(lat2)) * Math.sin(dLng / 2) ** 2;
	const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
	return Math.round(R * c);
}
var STORAGE_KEY = "sentinel_work_mode";
function loadInitial() {
	if (typeof window === "undefined") return { mode: "office" };
	try {
		const raw = localStorage.getItem(STORAGE_KEY);
		if (raw) return JSON.parse(raw);
	} catch {}
	return { mode: "office" };
}
/**
* Tiny external store (no extra deps) so multiple components can read/write
* the current work mode and stay in sync via useSyncExternalStore.
*/
var WorkStore = class {
	state = loadInitial();
	listeners = /* @__PURE__ */ new Set();
	get = () => this.state;
	set = (partial) => {
		this.state = {
			...this.state,
			...partial
		};
		if (typeof window !== "undefined") localStorage.setItem(STORAGE_KEY, JSON.stringify(this.state));
		this.listeners.forEach((l) => l());
	};
	subscribe = (listener) => {
		this.listeners.add(listener);
		return () => this.listeners.delete(listener);
	};
};
var workStore = new WorkStore();
function useWorkMode() {
	return (0, import_react.useSyncExternalStore)(workStore.subscribe, workStore.get, workStore.get);
}
/**
* An employee is no longer free to pick any vendor — they are bound to
* exactly one vendor location by the admin (Employee.vendor_id). This just
* resolves that binding.
*/
function getVendor(vendorId) {
	return getVendorById(vendorId);
}
//#endregion
export { workStore as a, useWorkMode as i, getVendor as n, haversine as r, OFFICE as t };
